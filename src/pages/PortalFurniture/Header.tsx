...
function Header({
  ...
  onSearch: (query: string, type: "name" | "shop") => void;
}) {
    ...
    <div className="text-background max-w-6xl mx-auto pt-16 px-4 sm:px-6 md:px-8 flex flex-col items-center text-center md:text-left">
      <div className="w-full flex flex-col md:flex-row items-center justify-between">
        <div className="max-w-2xl">
          <p className="text-sm font-semibold mt-8">Nie wiesz, od czego zacząć?</p>
          <h1 className="text-xl md:text-2xl font-bold mt-2">
            Znajdź produkty, które odmienią Twoją przestrzeń!
          </h1>
          ...
          <div className="mt-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                className="border border-background rounded-lg px-2 py-3 bg-transparent focus:outline-none focus:ring-2 focus:ring-brown"
                value={searchType}
                onChange={(e) => setSearchType(e.target.value as "name" | "shop")}
              >
                <option value="name">nazwie</option>
                <option value="shop">sklepie</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 md:mt-0 md:ml-8">
          <img src={logo} className="w-[200px] h-auto mt-8 md:mr-6" />
        </div>
      </div>
