...
import {
  useGetApiPortalModelObjectId,
  useGetApiPortalObjectId,
...
interface PublicFurnitureViewerProps {
  isMobile: boolean;
}

export default function PublicFurnitureViewer({
  isMobile,
}: PublicFurnitureViewerProps) {
  ...
  } = useGetApiPortalModelObjectId(id?.toString() || "");

  const objectRequest = useGetApiPortalObjectId(id?.toString() || "");

  useEffect(() => {
    if (
      (!objectRequest.data?.public || objectRequest.data?.error !== null) &&
      !objectRequest.isLoading
    ) {
      navigate("/not-found");
      return;
    }
  }, [objectRequest, navigate]);
