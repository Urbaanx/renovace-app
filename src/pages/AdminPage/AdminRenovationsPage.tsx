import { useGetAdminGetRenovations } from "../../api/endpoints/api";
...
function AdminRenovationsPage() {
  ...
  const [orderByCol, setOrderByCol] = useState("renovationid");
  const [renovationIdFilter, setRenovationIdFilter] = useState("");
  const [nameFilter, setNameFilter] = useState("");
  const [modificationDateFilterLower, setModificationDateFilterLower] =
    useState("");
  const [modificationDateFilterUpper, setModificationDateFilterUpper] =
    useState("");
  const [creationDateFilterUpper, setCreationDateFilterUpper] = useState("");
  const [creationDateFilterLower, setCreationDateFilterLower] = useState("");
  const [editable, setEditable] = useState(-1);
  const [filterFocus, setFilterFocus] = useState("");

  const query = useGetAdminGetRenovations({
    page: page,
    pageSize: pageSize,
    orderByAsc: orderByAsc,
    orderBycol: orderByCol,
    renovationIdFilter: renovationIdFilter,
    nameFilter: nameFilter,
    modificationDateFilterLower: modificationDateFilterLower,
    modificationDateFilterUpper: modificationDateFilterUpper,
    creationDateFilterLower: creationDateFilterLower,
    creationDateFilterUpper: creationDateFilterUpper,
  });
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-8 bg-background pl-16 grid-rows-9 h-dvh w-dvw">
        <Header headerText="Renovation"></Header>
        <div className="hidden xl:block col-span-1 row-span-8">
          <Menu current="renovation"></Menu>
        </div>
        <div className="col-span-8 xl:col-span-7 row-span-8 px-10 py-20 grid grid-cols-5 rows-12 text-mainColorText overflow-clip">
          <div className="row-span-2 col-span-5 lg:grid grid-cols-5 overflow-scroll hidden ">
            ...
            <input
              value={renovationIdFilter}
              autoFocus={filterFocus == "renovationId"}
              placeholder="renovationId"
              className="text-background my-5 h-1/2 mr-16 rounded-md row-start-2 max-h-8 min-w-28"
              onChange={(e) => {
                setRenovationIdFilter(e.target.value);
                setFilterFocus("renovationId");
              }}
            />
            <input
              value={nameFilter}
            ...
            <div className="row-start-2 flex">
              <input
                autoFocus={filterFocus == "modificationLower"}
                type="datetime-local"
                value={modificationDateFilterLower}
                className="text-background my-5 h-1/2 mr-8 rounded-md max-h-8 w-5/12 min-w-28"
                onChange={(e) => {
                  setModificationDateFilterLower(e.target.value);
                  setFilterFocus("modificationLower");
                }}
              />
              <input
                autoFocus={filterFocus == "modificationUpper"}
                type="datetime-local"
                value={modificationDateFilterUpper}
                className="text-background my-5 h-1/2 mr-16 rounded-md max-h-8 w-5/12 min-w-28"
                onChange={(e) => {
                  setModificationDateFilterUpper(e.target.value);
                  setFilterFocus("modificationUpper");
                }}
              />
            ...
            <div className="row-start-2 flex">
              <input
                autoFocus={filterFocus == "creationLower"}
                type="datetime-local"
                value={creationDateFilterLower}
                className="text-background my-5 h-1/2 mr-8 rounded-md max-h-8 w-5/12 min-w-28"
                onChange={(e) => {
                  setCreationDateFilterLower(e.target.value);
                  setFilterFocus("creationLower");
                }}
              />
              <input
                autoFocus={filterFocus == "creationUpper"}
                type="datetime-local"
                value={creationDateFilterUpper}
                className="text-background my-5 h-1/2 mr-16 rounded-md max-h-8 w-5/12 min-w-28"
                onChange={(e) => {
                  setCreationDateFilterUpper(e.target.value);
                  setFilterFocus("creationUpper");
                }}
              />
              <div className="w-1/5"></div>
            </div>
...
export default AdminRenovationsPage;
