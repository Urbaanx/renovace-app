import { useGetAdminGetRoomPermissions } from "../../api/endpoints/api";
import { useState } from "react";
import DataTableHeader from "./DataTableHeader";
import Paginator from "./Paginator";
import RoomPermissionsModal from "./RoomPermissionsModal";
import SideBar from "../../components/Sidebar/Sidebar";
import Header from "./Header";
import Menu from "./Menu";

function AdminRoomPermissionPage() {
  const tableHeaders = ["roomId", "userId", "renovationOnly", "canEdit"];
  ...
  const [renovationOnlyFilter, setRenovationOnlyFilter] = useState(false);
  const [canEditFilter, setCanEditFilter] = useState(true);
  const [editable, setEditable] = useState(-1);
  const [filterFocus, setFilterFocus] = useState("");

  const query = useGetAdminGetRoomPermissions({
    page: page,
    pageSize: pageSize,
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-8 bg-background pl-16 grid-rows-9 h-dvh w-dvw">
        <Header headerText="Room Permissions"></Header>
        <div className="hidden xl:block col-span-1 row-span-8">
          <Menu current="room permissions"></Menu>
        </div>
...
export default AdminRoomPermissionPage;
