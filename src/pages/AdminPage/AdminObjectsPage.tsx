import { useGetAdminGetObjects } from "../../api/endpoints/api";
...
function AdminObjectsPage() {
  const tableHeaders = ["objectId", "name", "generationDate"];
  ...
  const query = useGetAdminGetObjects({
    page: page,
    pageSize: pageSize,
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-8 bg-background pl-16 grid-rows-9 h-dvh w-dvw">
        <Header headerText="Object"></Header>
        <div className="hidden xl:block col-span-1 row-span-8">
          <Menu current="object"></Menu>
        </div>
        <div className="col-span-8 xl:col-span-7 row-span-8 px-10 py-20 grid grid-cols-5 rows-12 text-mainColorText overflow-clip">
          <div className="row-span-2 col-span-5 lg:grid grid-cols-4 overflow-scroll hidden ">
            ...
            <button
              className="place-self-end my-5 bg-mainColorText font-bold text-background rounded-md py-1 px-2 row-start-2 col-start-4 max-h-8 hover:brightness-150 w-20 xl:w-36 2xl:w-44"
