...
import { ObjectInRenovation } from "../../api/endpoints/api.schemas";
import "../../styles.css";
import Modal from "../HomePage/subpages/Home/Modal";

interface ObjectInRenovationModalProps {
  setIsOpen: (val: boolean) => void;
  values: ObjectInRenovation | undefined;
  refresh: () => void;
}

function ObjectInRenovationModal({
  setIsOpen,
  values,
  refresh,
}: ObjectInRenovationModalProps) {
  ...
  const updateQuery = usePutApiObjectsInRenovations();
  const addQuery = usePostApiObjectsInRenovations();
  const deleteQuery = useDeleteApiObjectsInRenovationsId();
    ...
    >
      <div className="mx-3 ml-0 h-auto overflow-y-scroll bg-mainColorText text-background scrollable-admin grid grid-cols-3 rounded-2xl gap-3 p-5 pt-0">
        <div className="sticky top-0 col-span-3 bg-mainColorText py-4">
          {values == undefined && (
            <h1 className="text-xl text-center">Dodaj Object In Renovation</h1>
          )}
          {values != undefined && (
            <h1 className="text-xl text-center">
              Uaktualnij Object In Renovation
            </h1>
          )}
        </div>
          ...
          onClick={async () => {
            if (values != null && values.id != undefined) {
              await updateQuery.mutateAsync({
                ...
                params: {
                  objectInRenovationId: values.id,
                },
              });
            } else {
...
export default ObjectInRenovationModal;
